...
exports.newDoctor = async (req, res) => {
  ...
  try {
    ...
    const userId = decoded.id;
    await db.query(
      "INSERT INTO doctor(firstname, lastname, typeofdoc, phonenumber, location, notes, user_id) VALUES ($1, $2, $3, $4, $5, $6, $7)",
...
exports.editDoctor = async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "UPDATE doctor SET firstname = $1, lastname = $2, typeofdoc = $3, phonenumber = $4, location = $5, notes = $6 WHERE doctor_id = $7 AND user_id = $8",
      [
        firstname,
        lastname,
        typeofdoc,
...
exports.getMedicine = async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(
      "SELECT m.medicine_id, m.name, m.function, m.frequency, m.doctor_id, m.user_id, m.other_prescriber, m.prescription, d.firstname, d.lastname, d.typeofdoc, d.phonenumber, d.location, d.notes FROM medicine AS m LEFT JOIN doctor AS d ON m.doctor_id = d.doctor_id WHERE m.user_id = $1",
      [userId]
    );
    res.json(rows);
  } catch (err) {
...
exports.newMedicine = async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "INSERT INTO medicine(name, function, frequency, doctor_id, user_id, other_prescriber, prescription) VALUES ($1, $2, $3, $4, $5, $6, $7)",
...
exports.editMedicine = async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "UPDATE medicine SET name = $1, function = $2, frequency = $3, prescription = $4, doctor_id = $5, other_prescriber = $6 WHERE medicine_id = $7 AND user_id = $8",
      [
        name,
        func,
        frequency,
...
exports.deleteMedicine = async (req, res) => {
  ...
  try {
    ...
    const medicineId = req.params.id;
    await db.query(
      "DELETE FROM medicine WHERE medicine_id = $1 AND user_id = $2",
      [medicineId, userId]
    );
...
exports.newAppointment = async (req, res) => {
  ...
  try {
    ...
    const userId = decoded.id;
    await db.query(
      "INSERT INTO appointment(title, date, time, prenotes, postnotes, doctor_id, user_id, other_professional) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
      [
        title,
        date,
        time,
...
exports.editAppointment = async (req, res) => {
  ...
  try {
    ...
    const userId = decoded.id;
    await db.query(
      "UPDATE appointment SET title = $1, date = $2, time = $3, prenotes = $4, postnotes = $5, doctor_id = $6, other_professional = $7 WHERE user_id = $8 AND appointment_id = $9",
      [
        title,
        date,
        time,
...
exports.deleteAppointment = async (req, res) => {
  ...
  try {
    ...
    const appointmentid = req.params.id;
    await db.query(
      "DELETE FROM appointment WHERE user_id = $1 AND appointment_id = $2",
